...
switch (command) {
	...
	menu = 
`Hy 👋 ${pushname}, ${tampilUcapan} 😇

✉️ Source Code = https://bit.ly/Zero-YT7
👤 Author = Zero YT7
🌍 Lib = Baileys
...
☕ Device = Windows`
fake =
`Subscribe Channel Zero YT7 Support To 3K Subscribe Thanks`
...
"jpegThumbnail": image,
  ...
  }}, MessageType.buttonsMessage, {sendEphemeral: true, contextInfo: {"externalAdReply": { "title": `Hallo ${pushname}`, "body": `${tampilUcapan}`, "mediaType": "2", "mediaUrl": "https://youtu.be/ilrhJV_QMIE", "thumbnail": image,},mentionedJid:[sender]}, quoted : ftrol})
  break
case 'allmenu':
allmenu =
`*GROUP MENU*
...
`
               ...
               imeu = await zeroyt7.prepareMessage('0@s.whatsapp.net', prsimg, image, { thumbnail: Buffer.alloc(0) })
...
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/nulis?apikey=Kontolodon&text=${ini_txt}`).then((gambar) => {
zeroyt7.sendMessage(from, gambar, image, { quoted: ftrol, thumbnail: Buffer.alloc(0) })
})
break
case 'attp':
...
zeroyt7.sendMessage(from, repur, image, { quoted: ftrol})
break
case 'pornhub':
reply(mess.wait)
...
zeroyt7.sendMessage(from, porn, image, {caption: 'PORNHUB', quoted: ftrol})
break 
